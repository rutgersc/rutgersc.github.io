...
export function renderXItem(postId: string, dateViewed: string | null, onRemove?: () => void): HTMLLIElement {
...
export const renderChannelVideoItem = (video: ChannelVideo, watchedIndex: Map<string, { dateViewed: string }>): HTMLDivElement => {
  ...
  if (isShort) item.style.opacity = "0.45";
...
export function renderVideoItem(videoData: VideoData, dateViewed: string | null, options: RenderOptions = {}): HTMLLIElement {
  ...
  if (fixButton) rightGroup.appendChild(fixButton);
...
export function renderChannelGroups(channels: ChannelGroup[]): HTMLDivElement {
  ...
  channels.forEach(channel => {
    ...
    const applyIgnored = (ignored: boolean): void => {
    ...
    channelHeader.onclick = (e: MouseEvent) => {
      if (e.target === channelName || (e.target as HTMLElement).closest?.("button")) return;
