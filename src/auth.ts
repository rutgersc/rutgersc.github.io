...
export function updateMsalUi(): void {
  ...
  const optionsPanel = document.getElementById('msal-options-panel') as HTMLElement | null;
  if (!btn) return;
...
export function msalAcquireGraphToken(): Promise<string> {
  inFlightToken ??= acquireToken().finally(() => { inFlightToken = null; });
