...
export const renderChapters = (chapters, onSeek) => {
...
export const highlightCurrentChapter = (chapters, currentTime) => {
    const container = document.getElementById("chapters-container");
    if (!container)
        return;
    const activeIndex = chapters.reduce((acc, ch, i) => (ch.startSeconds <= currentTime ? i : acc), -1);
    container.querySelectorAll("button[data-chapter-index]").forEach(btn => {
