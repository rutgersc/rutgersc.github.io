...
function setupVolumeControls() {
    ...
    const renderVolume = () => {
        ...
        if (volumeFill)
            volumeFill.style.width = `${currentVol}%`;
    };
    const setVolume = (vol) => {
...
export async function apply_vid(vid, addHistory = true) {
    ...
    if (vid && player) {
        ...
        startSeek(100);
        const isStale = () => localStorage.getItem(CURRENT_X_POST_KEY) !== null || localStorage.getItem(CURRENT_VID_KEY) !== vid;
        const waitForPlayerData = async (attempts) => {
            const raw = player?.getVideoData();
            if (raw?.video_id === vid && raw.title && raw.author)
                return raw;
            if (attempts <= 1 || isStale())
                return raw?.video_id === vid ? raw : null;
